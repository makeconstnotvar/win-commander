...
TEST_CASE(PREFIX "Can unzip an archive with Chinese symbols")
{
    ...
    // This one was compressed with application version 1.4(?)
...
TEST_CASE(PREFIX "Can unzip an archive with Cyrillic symbols")
{
    ...
    // This one is compressed with application version 1.4
...
TEST_CASE(PREFIX "zip archive with a heading slash in a filename")
{
    ...
    // but many tools support these inconsistencies, so Duck Commander does as well. Unfortunately I don't know how to produce a
...
TEST_CASE(PREFIX "time handling")
{
    // touch test.txt
    ...
    //   test.txt
    //   [...]
    //   file last modified on (DOS date/time):          2026 Jun 15 17:15:18
    //   file last modified on (UT extra field modtime): 2026 Jun 15 17:15:17 local
    //   file last modified on (UT extra field modtime): 2026 Jun 15 16:15:17 UTC
    const unsigned char arc_zip[] = {
        };
    const TestDir dir;
    const auto path = std::filesystem::path(dir.directory) / "arc_zip";
    REQUIRE(nc::base::WriteAtomically(path, {reinterpret_cast<const std::byte *>(arc_zip), std::size(arc_zip)}));
    std::shared_ptr<ArchiveHost> host;
    ...
    CHECK(std::abs(st.mtime.tv_sec - expected_tv_sec) < _24h);
    CHECK(std::abs(st.ctime.tv_sec - expected_tv_sec) < _24h);
    CHECK(std::abs(st.btime.tv_sec - expected_tv_sec) < _24h);
    // Probe via FetchDirectoryListing()
    const VFSListingPtr listing = host->FetchDirectoryListing("/", VFSFlags::F_NoDotDot).value();
    CHECK(std::abs(listing->ATime(0) - expected_tv_sec) < _24h);
    CHECK(std::abs(listing->MTime(0) - expected_tv_sec) < _24h);
    CHECK(std::abs(listing->CTime(0) - expected_tv_sec) < _24h);
    CHECK(std::abs(listing->BTime(0) - expected_tv_sec) < _24h);
}

TEST_CASE(PREFIX "proxy typed acquisition opens a standard archive and preserves the legacy wrapper")
{
    static const unsigned char zip[] = {
        ...
        0x00, 0x01, 0x00, 0x4e, 0x00, 0x00, 0x00, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00};
    const TestDir dir;
    const auto path = std::filesystem::path(dir.directory) / "proxy.zip";
    REQUIRE(nc::base::WriteAtomically(path, {reinterpret_cast<const std::byte *>(zip), std::size(zip)}));

    const ArchiveOpenResult result = VFSArchiveProxy::OpenFileAsArchiveResult(path.native(), TestEnv().vfs_native);
    REQUIRE(result);
    CHECK(std::dynamic_pointer_cast<ArchiveHost>(*result) != nullptr);
    CHECK(VFSArchiveProxy::OpenFileAsArchive(path.native(), TestEnv().vfs_native) != nullptr);
}

TEST_CASE(PREFIX "proxy typed acquisition classifies missing and cancelled sources")
{
    const TestDir dir;
    const auto missing_path = std::filesystem::path(dir.directory) / "missing.zip";

    const ArchiveOpenResult missing =
        VFSArchiveProxy::OpenFileAsArchiveResult(missing_path.native(), TestEnv().vfs_native);
    REQUIRE_FALSE(missing);
    CHECK(missing.error().kind == ArchiveOpenFailureKind::SourceMissing);
    CHECK(missing.error().primary_error == nc::Error{nc::Error::POSIX, ENOENT});
    CHECK_FALSE(missing.error().fallback_error);
    CHECK(VFSArchiveProxy::OpenFileAsArchive(missing_path.native(), TestEnv().vfs_native) == nullptr);
    ...
    static const unsigned char zip[] = {
        ...
        0x01, 0x00, 0x00, 0x04, 0x14, 0x00, 0x00, 0x00, 0x50, 0x4b, 0x05, 0x06, 0x00, 0x00, 0x00, 0x00, 0x01,
        0x00, 0x01, 0x00, 0x4e, 0x00, 0x00, 0x00, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00};
    const auto path = std::filesystem::path(dir.directory) / "cancelled.zip";
    REQUIRE(nc::base::WriteAtomically(path, {reinterpret_cast<const std::byte *>(zip), std::size(zip)}));

    const ArchiveOpenResult cancelled =
        VFSArchiveProxy::OpenFileAsArchiveResult(path.native(), TestEnv().vfs_native, nullptr, [] { return true; });
    REQUIRE_FALSE(cancelled);
    CHECK(cancelled.error().kind == ArchiveOpenFailureKind::Cancelled);
    CHECK(cancelled.error().primary_error == nc::Error{nc::Error::POSIX, ECANCELED});
}

TEST_CASE(PREFIX "proxy typed acquisition classifies invalid input")
{
    static const unsigned char invalid[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05};
    const TestDir dir;
    const auto path = std::filesystem::path(dir.directory) / "invalid.zip";
    REQUIRE(nc::base::WriteAtomically(path, {reinterpret_cast<const std::byte *>(invalid), std::size(invalid)}));

    const ArchiveOpenResult result = VFSArchiveProxy::OpenFileAsArchiveResult(path.native(), TestEnv().vfs_native);
    REQUIRE_FALSE(result);
    CHECK(result.error().kind == ArchiveOpenFailureKind::InvalidOrUnsupportedArchive);
    CHECK_FALSE(result.error().fallback_error);
    CHECK(VFSArchiveProxy::OpenFileAsArchive(path.native(), TestEnv().vfs_native) == nullptr);
}

TEST_CASE(PREFIX "proxy typed acquisition falls back to a raw archive host")
{
    static const unsigned char gzip[] = {0x1f, 0x8b, 0x08, 0x08, 0xb6, 0x29, 0xef, 0x61, 0x00,
                                         0x03, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2e, 0x74, 0x78,
                                         0x74, 0x00, 0xcb, 0x48, 0xcd, 0xc9, 0xc9, 0x07, 0x00,
                                         0x86, 0xa6, 0x10, 0x36, 0x05, 0x00, 0x00, 0x00};
    const TestDir dir;
    const auto path = std::filesystem::path(dir.directory) / "hello.txt.gz";
    REQUIRE(nc::base::WriteAtomically(path, {reinterpret_cast<const std::byte *>(gzip), std::size(gzip)}));

    const ArchiveOpenResult result = VFSArchiveProxy::OpenFileAsArchiveResult(path.native(), TestEnv().vfs_native);
    REQUIRE(result);
    CHECK(std::dynamic_pointer_cast<ArchiveRawHost>(*result) != nullptr);
    const VFSListingPtr listing = (*result)->FetchDirectoryListing("/", VFSFlags::F_NoDotDot).value();
    REQUIRE(listing->Count() == 1);
