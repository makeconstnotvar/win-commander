...
[[clang::no_destroy]] static dispatch_queue g_Queue{"com.wincommander.footer_fs_stat"};
...
struct PanelViewFooterVolumeInfoFetcherInternals {
    ...
    static void AcceptResult(VFSHostWeakPtr _host, std::string_view _path, const std::optional<VFSStatFS> &_stat)
    {
        ...
        for( auto &lp : g_Context )
            if( equals(lp.host, _host) && lp.path == _path ) {
                lp.scheduled = false;
    ...
    static const VFSStatFS *
    RegisterWatcher(FooterVolumeInfoFetcher *_w, const VFSHostWeakPtr &_host, const std::string &_path)
    {
        dispatch_assert_main_queue();
    ...
    static const VFSStatFS *
    Probe([[maybe_unused]] FooterVolumeInfoFetcher *_w, const VFSHostWeakPtr &_host, const std::string &_path)
    {
        dispatch_assert_main_queue();
    ...
    static void RemoveWatcher(FooterVolumeInfoFetcher *_w, const VFSHostWeakPtr &_host, const std::string &_path)
    {
        dispatch_assert_main_queue();
