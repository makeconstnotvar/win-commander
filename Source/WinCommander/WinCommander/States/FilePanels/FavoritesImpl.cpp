...
FavoriteLocationsStorageImpl::ComposeFavoriteLocation(VFSHost &_host,
...
FavoriteLocationsStorageImpl::FrecentlyUsed(int _amount) const
{
    ...
    for( auto &v : m_Visits )
        if( v.second.last_visit > last_date && v.second.visits_count > 0 && !is_favorite(v.first) )
            recent_visits.emplace_back(v.first, v.second.visits_count, v.second.last_visit, 0.);
    ...
    for( int i = 0, e = std::min(_amount, static_cast<int>(recent_visits.size())); i != e; ++i )
...
std::optional<FavoriteLocationsStorage::Favorite>
FavoriteLocationsStorageImpl::JSONToFavorite(const config::Value &_json)
{
    if( !_json.IsObject() )
        return std::nullopt;
...
void FavoriteLocationsStorageImpl::StoreData(config::Config &_config, const char *_path)
{
    ...
    json.AddMember(MakeStandaloneString("automatic"), std::move(automatic), g_CrtAllocator);
...
FavoriteLocationsStorageImpl::ObservationTicket
FavoriteLocationsStorageImpl::ObserveFavoritesChanges(std::function<void()> _callback)
{
