# План работ: редизайн Duck Commander в стиле Windows Explorer
...
Все ссылки на файлы даны относительно корня репозитория (`Source/...`). Они получены прямым чтением исходников, поэтому на них можно ориентироваться при декомпозиции задач в issue-трекере.
...
- по **функциональности и механике** — это Duck Commander (VFS, операции, вьювер, терминал, темы, конфиги, VFS-архивы/FTP/SFTP/WebDAV/xattr/processes);
...
- отказ от «фишек» Duck Commander (двухпанельный режим, VFS-архивы, batch rename, quick search, вкладки) — они остаются и получают новую визуальную упаковку и новые точки входа в духе Explorer.

**Лицензия.** Базовый код распространяется под GPLv3 (`LICENSE.md`). Проект сохраняет GPLv3, copyright-уведомления и атрибуцию авторского кода © Michael Kazakov. GPLv3 допускает отдельный бренд и требует открытости исходников производного продукта.

**Бренд.** Продукт носит имя Duck Commander; иконка, bundle ID и все пользовательские точки входа согласованы с этим именем.
...
| Sidebar / Navigation Pane | — | **new** | В Duck Commander нет постоянного sidebar — вся навигация через модальные popup’ы. Новый компонент строится на тех же источниках данных, что и Go-To popup (см. выше), плюс `NativeFSManager` (тома → «Этот Mac»), `NetworkConnectionsManager` (сеть), `TagsStorage` (метки). |
...
| Иконка приложения, bundle ID, имя | `Source/WinCommander/WinCommander.xcodeproj/project.pbxproj`, `Resources/*-Info.plist`, `Resources/Icon.icns` | **new identity** | Единая идентичность Duck Commander для всех вариантов сборки. |
...
1. **Визуальное состояние Cut («вырезано»).** В Duck Commander уже есть Copy/Move через pasteboard (`Cmd+C`/`Cmd+V`/`Opt+Cmd+V`), но нет предварительной пометки «вырезанного» элемента (в Explorer вырезанные иконки становятся полупрозрачными до вставки/отмены). Нужен: (а) набор «помечено как cut» на стороне UI-контроллера панели, (б) флаг в presentation-слое (`NCPanelViewPresentationProtocol`-реализациях) для затемнения иконки. Данные не трогаем, только рендер + маленький стейт в контроллере.
...
3. **(Опционально/stretch) Один шаг Undo для последней операции.** В Windows Explorer `Ctrl+Z` отменяет последнее файловое действие. В Duck Commander это новая механика, которую следует вынести в бэклог после стабилизации визуального слоя (см. §6, Фаза 15, «дорожная карта после релиза»).
...
6. **Стабильный development channel.** Локальные UI-проверки фаз выполняются через `Scripts/build_stable_dev_and_run.sh`. Канонические path, bundle ID и designated requirement сохраняются между фазами; production rebranding и variant-specific release checks развиваются отдельно от `com.wincommander.App.CodexDev` на фактических signed artifacts.

---

## 6. Фазы и этапы (roadmap)

Оценки — для одного full-stack macOS/AppKit-разработчика, работающего с постоянной вовлечённостью. При частичной занятости (10–20 ч/нед) сроки нужно умножать на 2–3.
...
| 16 | Ребрендинг и дистрибуция | Bundle ID, иконка и имя Duck Commander; обновление `Info.plist` (MAS и non-MAS), CI (`build.yml`/`nightly.yml` как стартовая точка), README/CONTRIBUTING/NOTICE с атрибуцией базового кода | `*.pbxproj`, `Resources/*-Info.plist`, `.github/workflows/*` | Подписанная сборка ставится и обновляется (Sparkle/App Store — по выбору) | 2 нед |
...
|---|---|---|
| Сохранять ли двухпанельный режим? | Это ключевая механика Duck Commander (`Shift+Cmd+P` переключает dual/single-pane уже сегодня). Explorer — однопанельный. | Сохранить «Split View» как один клик/хоткей для опытных пользователей, а по умолчанию открываться в одиночном режиме. |
...
| Совместимость конфигов при апгрейде | `Source/Config` хранит настройки в JSON под `~/Library/Application Support/<CFBundleExecutable>/Config`. | Сохранять техническое имя executable как контракт совместимости; все UI-поверхности используют Duck Commander. |
| MAS (App Store) vs прямая дистрибуция | Сегодня поддерживаются оба варианта (`WinCommander-MAS-Info.plist`/`WinCommander-NonMAS-Info.plist`), с урезанным функционалом в песочнице (нет Admin Mode, терминала, монтирования сетевых шар — см. `Docs/Help.md`, «Version Differences»). | Прямая дистрибуция первична для полного набора функций; MAS остаётся песочным вариантом. |
| Ограниченность ресурсов (потенциально 1 контрибьютор) | Текущая модель поддержки описана в FAQ. | Приоритизировать Фазы 1–7 (то, что визуально «продаёт» идею) и Фазу 16 (дистрибуция), Фазы 10/13/14 можно сдвигать. |
...
- Для AppKit-рендеринга используется чек-лист и ручное скриншот-сравнение перед релизом (см. `Docs/ScreenshotsAutomation`).

---

## 9. Открытие исходного кода

- Лицензия остаётся **GPLv3** (`LICENSE.md`), с сохранением copyright-заголовков базового кода.
- В README/CONTRIBUTING/NOTICE — атрибуция базового кода (© Michael Kazakov) с описанием изменённого UI-слоя и переиспользуемого движка.
