...
| Tests/CI | **partial** | Current-tree local M0 gate прошёл unsigned Debug build и 10/10 aggregate binaries: 897 / 132 011. Current full `OperationsUT` passes 165 / 4 468 in Debug and explicitly instrumented Release ASAN/UBSAN with confirmed runtimes and no diagnostics; current full Debug `VFSUT` is 94 / 43 531. Focused Operations evidence: journal 27 / 592, Native create-copy 19 / 924, Native outcome mapper 2 / 382, provider journal mapper 4 / 237, product 9 / 188, factory 8 / 225, Job 10 / 608, probes 5 / 178, orchestrator 13 / 558 (production 3 / 138), Pool 15 / 190. ProviderCapabilities and Native conditional Copy pass 16 / 548 and 15 / 312. Seeded Docker-backed ASAN integration passes 163 / 89 392. Hosted CI remains open. |

Current-tree M0 2026-08-01: `Scripts/verify_m0.sh` собрал unsigned Debug application и выполнил все десять seeded aggregate unit-test targets — 897 cases / 132 011 assertions в recorded run. Current `OperationsUT` passes 165 / 4 468 in Debug and explicitly instrumented Release ASAN/UBSAN. Seeded Docker-backed integration ASAN passed 163 / 89 392; the first hosted CI run remains open evidence.
...
- `Source/VFS/include/VFS/ProviderCapabilities.h` и resolver дают conservative path-aware projection для NativeHost, ArcLA, ArcLARaw, FTP, SFTP, WebDAV, XAttrHost и PSHost. `VFSHost` классифицирует POSIX failures, SFTP дополняет provider-domain classification; authoritative case semantics, semantic namespace identity и symlink creation/read capability поступают в preflight fail closed. Move-only reviewed authority privately retains the consumed exact preflight and issues immutable Copy claims once. Native conditional transaction owns source/destination-parent descriptors, admits exact same internal/local/writable APFS volume, seals supported metadata, publishes through one `fclonefileat(..., CLONE_ACL)`, verifies the destination and executes ordered durability barriers; other scopes remain `Unsupported`. Current focused ProviderCapabilities passes 16 / 548, Native conditional Copy 15 / 312, combined Debug selection 31 / 860; current full Debug `VFSUT` passes 94 / 43 531, and seeded Docker-backed VFS/VFSIcon ASAN integration passed 58 / 87 975;
...
**Текущее evidence:** journal 27 / 592, Native create-copy 19 / 924, Native mapper 2 / 382, provider journal mapper 4 / 237, product 9 / 188, factory 8 / 225, Job 10 / 608, probes 5 / 178, orchestrator 13 / 558 with production subset 3 / 138, Pool 15 / 190, ProviderCapabilities 16 / 548 and Native conditional Copy 15 / 312. Full `OperationsUT` passes 165 / 4 468 in Debug and explicit Release ASAN/UBSAN; both runtimes were confirmed with no diagnostics. Full Debug `VFSUT` passes 94 / 43 531. Current-tree M0 remains 897 / 132 011; seeded Docker-backed integration ASAN passes 163 / 89 392. Production application consumer is absent.

**Оставшееся evidence:** one app-owned Copy consumer with typed bound-plan review, pre-enqueue observer configuration, zero enqueue for blocked/stale/cancelled/unpersisted intent and durable terminal publication before Pool removal; bounded cross-volume staging; physical internal/external-volume and power-loss matrix; Operation Center persistence; non-Copy preflight; hosted required CI. Docker-backed integration baseline does not include the production volume fixtures.
...
1. Подключить один production Copy boundary: structural plan → VFS/application preflight → typed review/confirmation → journal admission/run → private reviewed factory/provider transaction → `Pool` → durable item/terminal result. Сначала добавить pre-enqueue operation configurator/terminal presentation seam, чтобы UI observers регистрировались до `Start`.
2. Доказать app-owned boundary tests: zero enqueue for blocked/stale/cancelled/unpersisted intent, exact typed presentation and durable item/terminal publication before Pool removal.
3. Добавить dedicated internal/external-volume `OperationsIT` и power-loss evidence.
4. Реализовать bounded private staging authority для отдельного cross-volume scope.
5. После одного consumer расширять factory на batch и остальные P0 mutation consumers; параллельно переводить оставшиеся P0 command consumers на Registry/Capabilities/Error/Visual State contracts.
6. Расширить production lifecycle evidence на live local/remote providers и permission/deferred-Busy cases; подключить status bar, preview/details, tabs и persistence в M2.
7. Получить первый hosted run `M0 Verification` после публикации и закрепить выполненный Docker-backed ASAN baseline как отдельный required integration profile.
