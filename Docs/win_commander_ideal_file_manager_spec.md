# Duck Commander: спецификация идеального файлового менеджера

**Версия:** 1.0  
**Статус:** каноническая продуктовая и архитектурная спецификация
...
**Исполнение:** этапы, текущий статус, зависимости и evidence ведутся в [Development Plan](Development-Plan.md). Эта спецификация определяет, что строится и каким требованиям соответствует результат; Development Plan определяет порядок реализации.

---

## 0. Как агент должен использовать этот документ

Этот документ является каноническим рабочим брифом для проектирования и реализации Duck Commander.
...
Duck Commander — это файловый менеджер нового поколения для macOS.
...
| Боль пользователей | Требование Duck Commander |
|---|---|
...
Duck Commander получает оценку 10/10 только если выполняет все условия:
