...
| Operations | per-window `Operations::Pool` + per-operation `Operation`/`Statistics`; process-owned `OperationJournal`, receipt custodian and recovery coordinator store admitted/running/interrupted/terminal records, tri-state item results and bounded retry/reconcile/release slots | restricted cold-operation hooks; owning durable outcome; journal snapshots; coordinator Retry/Reconcile/ReleaseReconciled; Pool Finalizing residency and retry | Production-configured orchestrator links the private factory, transaction result, journal and Pool. Bounded `CopyAs` supplies exact review, window submission custody and typed UI dispatch. Operation Center later aggregates pools and durable records. |
...
| Provider and listing semantics | Current Debug: [`ProviderCapabilities_UT.cpp`](../Source/VFS/tests/ProviderCapabilities_UT.cpp) 16 / 549 and [`NativeConditionalCopyTransaction_UT.cpp`](../Source/VFS/tests/NativeConditionalCopyTransaction_UT.cpp) 16 / 328; combined 32 / 877; latest full `VFSUT` run 95 / 43,566. Prior focused Release ASAN/UBSAN snapshots passed 16 / 548 and 15 / 312. Seeded Docker-backed ASAN VFS/VFSIcon integration remains 58 / 87,975. | dedicated physical internal/external-volume and power-loss fixtures, cross-volume staging, richer Unicode/path identity, live provider identity and disconnected cases. |
| Operations plan/queue/lifecycle | Focused Debug: factory 8 / 225, probes 5 / 178, Native create-copy 19 / 924, Native mapper 2 / 382, provider mapper 4 / 237, product 9 / 188, Job 10 / 608, journal 27 / 592, orchestrator 15 / 758 and Pool 17 / 219; CopyAs policy/gate 6 / 28 and recovery coordinator 6 / 67. Full `OperationsUT` passes 170 / 4,748 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes and no diagnostics; Docker-backed Operations ASAN integration remains 87 / 862. | Live app-boundary tests, cross-volume staging/recovery, physical-volume/power-loss fixtures, lifecycle projection and partial-failure presentation. |
...
Current M3 `OperationsUT` evidence passes 170 / 4,748 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes and no diagnostics. The latest full Debug `VFSUT` run passes 95 / 43,566; current full Debug `WinCommanderUT` passes 309 / 4,995. Recorded `Scripts/verify_m0.sh` passed all ten aggregate binaries at 897 / 132,011. Seeded Docker-backed ASAN integration passes 163 / 89,392. Hosted `M0 Verification` remains open.
...
3. Command migration has nine production definitions from eleven stable IDs. Operations foundations now include the lossless mapper, sealed product, private factory path, production-configured orchestrator, exact receipt custody, restricted cold hooks, owning durable-outcome delivery and a preallocated Pool durable-finalization barrier. Bounded `CopyAs` is the first production mutation consumer; the next gates are live boundary proof, physical-volume evidence and cross-volume staging.
