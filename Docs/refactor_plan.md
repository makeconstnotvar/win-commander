...
**Current tests:** schema-v3 journal 33 / 752; pure OperationCenterModel 4 / 64; receipt-bound `OperationCenterCoordinator` with cold exact-storage history refresh 15 / 284; coordinator/orchestrator/control integration 31 / 1,060; confirmed recovery-to-history projection 8 / 107 in Debug, Release ASAN and Release UBSAN; Native create-copy 19 / 924; Native mapper 2 / 382; provider mapper 4 / 237; product 9 / 188; factory 8 / 225; Job 10 / 608; probes 5 / 178; orchestrator 19 / 849; Pool 17 / 219; ProviderCapabilities 16 / 549; Native conditional Copy 16 / 328; reviewed CopyAs policy/app boundary 10 / 98, including exact review, blocked/stale/unpersisted/cancelled zero enqueue and durable dispatch-before-release at 4 / 70 in Debug, Release ASAN and Release UBSAN. The physical `OperationsIT` tag compiles and skips two cases without supplied roots. Fresh isolated Debug `OperationsUT` is 195 / 196 and 5,270 / 5,274, with the same NativeCreateCopy set-ID metadata host baseline. Recorded M0 remains 897 / 132,011; Docker-backed ASAN remains 163 / 89,392.

**Remaining tests:** physical internal/external-volume execution and checkpoint-driven hardware power-loss evidence, isolated helper-backed cross-volume staging, and broader mutation-consumer adoption.
...
6. Route the implemented revision-checked engine cancellation through `CommandRegistry` with `OperationId` and the record's expected revision; presentation and Registry handlers never receive a raw `Operation *`. Then add pause/resume/retry only with their own sealed engine authority.
...
The previous M3 `OperationsUT` snapshot passed 170 / 4,748 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes and no diagnostics. The current coordinator/control batch passes 195 / 196 cases and 5,270 / 5,274 assertions in isolated Debug; its focused Release ASAN and UBSAN subset passes 31 / 1,060 without diagnostics. Confirmed recovery-to-history projection passes 8 / 107 in Debug, Release ASAN and Release UBSAN. The existing host-specific set-ID metadata baseline remains the sole full-Debug Operations failure. The latest full Debug `VFSUT` run passes 95 / 43,566; current full Debug `WinCommanderUT` passes 316 / 320 cases and 5,126 / 5,130 assertions, with four unrelated AppKit pasteboard-null baseline failures. Recorded M0 remains 897 / 132,011 and Docker-backed ASAN integration remains 163 / 89,392. Hosted CI publication and required-check enforcement remain separate work.
