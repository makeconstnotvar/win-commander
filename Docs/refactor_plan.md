...
**Статус 2026-08-02:** partial — pure registry/value layer, eleven stable IDs и `LegacyShortcutBindingAdapter` готовы. Десять app-owned production definitions покрывают `file.copy`, `file.cut`, initiation `file.rename`, `file.open`, `view.toggleHiddenFiles`, `navigation.back`, `navigation.forward`, `navigation.up`, `navigation.refresh` и `operation.cancel`. `file.open` объединяет main-menu/`OnOpenNatively:`, ordinary-file fallback of Enter, explicit Shortcut/Shift-Return и context-menu exact-item payloads; Enter сохраняет routing folder/archive navigation и terminal execution. Hidden-files and navigation commands converge across their menu, persisted shortcut and Explorer surfaces; Explorer `More` adds a compact immutable Operations snapshot and routes Cancel through the Registry; Up/Refresh toolbar presentation is matching-Store-backed and execution rechecks live controller state through the shared admission/availability queue-ownership helper. The secondary Up shortcut retains its menu bounce, and command descriptor aliases preserve Shortcut classification. Rename filesystem execution and History listing restore remain legacy. Параллельно продолжается migration оставшихся P0 commands and `PaneStore` consumers.
...
**Cross-volume foundation:** `ProviderConditionalCopyPathSupport` distinguishes `SameVolumeClone` and `CrossVolumeStaged`. Native routes the latter only through an injected descriptor/seal-only authority; the private client samples cancellation before Begin, after Granted and before Commit dispatch. The private client/transport seam exists, but no production helper transport is installed, so known cross-volume eligibility remains legacy. An installed unavailable authority fails closed prior to selection. `RoutedIO/CrossVolumeStagingProtocol` fixes an independent V1 wire contract for exact scalar seals, byte-component naming, correlation-bound opaque leases and the exact provider-compatible tri-state/error/sync completion matrix. Its private XPC codec owns exactly two duplicated FDs plus request-kind/reply grammar, while the injectable VFS client validates replies, keeps the one-use lease terminal and returns `Unknown` after any post-dispatch uncertainty. `CrossVolumeStagingHelperV1` links only the protocol/codec plus Security/CoreFoundation and validates every Begin's duplicated FDs as close-on-exec/read-only exact source/destination-parent scalar seals before inertly rejecting it. The move-only `ValidatedBegin` is the only argument accepted by its fixed private lease store, which mints token bytes with `SecRandomCopyBytes`, permits one live correlation for its owner, transfers validated FDs to one terminal claim and destroys them on revoke or rejected admission; their filters pass 4 / 208 and 5 / 328. The helper-private protected-root ledger accepts only a root-owned `0700` root FD, maintains one process-local owner per sealed root, and reads sealed artifacts only when exact root/header/role/artifact-ID claims plus the full no-follow artifact seal match. It retains every sealed, absent, incomplete, malformed or mismatched record, and removes a reservation only after its canonical artifact name is absent; its filter passes 8 / 207 in Debug, Release ASAN and Release UBSAN. Artifact materialization, seal writing, cleanup and namespace staging remain future mutable authority. The dispatcher has no store integration or mutation authority. NonMAS contains the V1 target dependency, `CodeSignOnCopy` and reciprocal privileged-helper requirements; Unsigned and MAS retain separate scope. The target configuration requires Developer ID and hardened runtime, while signed artifact/SMJobBless proof awaits an available signing identity. Native RAII aborts a granted lease until generic mint/ownership succeeds. Focused staging evidence: protocol 5 / 44, codec 4 / 74, client 4 / 108 and helper-target Debug build. Final descriptor/destination/stage revalidation plus cancellation at publication and physical evidence are separate layers.

**Remaining tests:** physical internal/external-volume execution, actual power interruption/reboot with the checkpoint manifest, pre-open journal capture and destination inspection, separately signed helper-backed cross-volume staging with helper-owned cleanup/recovery, and broader mutation-consumer adoption.
...
5. **First compact consumer done:** Explorer `More` reads one immutable `OperationCenterModel` snapshot and renders value-only type/state/ID lines with a Cancel child item. It is a bounded snapshot menu, not a live screen; unavailable and empty model states are explicit. Build the full Explorer Operation Center from `OperationCenterModel`, including live projection, persistent history/result detail and aggregate policy; preserve the existing brief Pool view only as legacy Commander compatibility during migration.
...
The historical M3 `OperationsUT` snapshot passed 170 / 4,748 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes and no diagnostics. The pre-checkpoint coordinator/control snapshot passed 195 / 196 cases and 5,270 / 5,274 assertions in isolated Debug; its focused Release ASAN and UBSAN subset passes 31 / 1,060 without diagnostics. Confirmed recovery-to-history projection passes 8 / 107 in Debug, Release ASAN and Release UBSAN. Current full Debug results are `OperationsUT` 197 / 5,295, `VFSUT` 127 / 130 and 44,616 / 44,619 with three host-environment baselines, and `WinCommanderUT --rng-seed 424242` 333 cases / 5,376 assertions. Recorded M0 remains 897 / 132,011 and Docker-backed ASAN integration remains 163 / 89,392. Hosted CI publication and required-check enforcement remain separate work.
