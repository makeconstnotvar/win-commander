...
**Статус 2026-08-02:** partial — structural plan through reviewed token, schema-v1 codec, private provider authority, bounded Native clone publication, lossless provider-result mapper, sealed transaction-backed Job and exact journal receipts are implemented. The production-configured orchestrator reaches the private reviewed factory; the public compatibility path still aborts and fails closed. Receipt custody covers pre-rename retry, same-storage read-only reconciliation and exact Pool release handshake. No app mutation consumer calls this boundary. Remaining gates are typed app review, pre-enqueue observer/terminal presentation, one mutation consumer, cross-volume staging and physical-volume fixtures.
...
4. Add application access composition and explicit reviewed authority. **Provider authority and private execution integration done.** The move-only private-sealed authority consumes the exact reviewed preflight and Native consumes it into a bounded transaction. Public `ReviewedOperationFactory::Create` remains a deliberate aborting compatibility path; the private orchestrator friend returns the sealed product. User-facing typed review remains open.
...
9. Change `CopyAs::Perform` first for one focused regular Native create-only item. Add an app review summary, a cold pre-enqueue configurator, and an exact durable terminal presenter before wiring `intent → plan → preflight → review → journal → private factory → Pool → durable outcome`. Other copy entry points remain legacy until their broader scope is planned.
...
**Current tests:** journal 27 / 592; Native create-copy 19 / 924; Native mapper 2 / 382; provider mapper 4 / 237; product 9 / 188; factory 8 / 225; Job 10 / 608; probes 5 / 178; orchestrator 13 / 558; Pool 15 / 190; ProviderCapabilities 16 / 548; Native conditional Copy 15 / 312. Full `OperationsUT` passes 165 / 4,468 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes. Full Debug `VFSUT` passes 94 / 43,531; M0 remains 897 / 132,011; Docker-backed ASAN remains 163 / 89,392.

**Remaining tests:** typed app review, pre-enqueue observer/terminal presentation, one production `CopyAs` boundary, bounded cross-volume staging and physical internal/external-volume plus power-loss fixtures.
...
| 4c | production VFS/application probes + bound preflight | production evidence boundary, no mutation consumer | completed: adapter 5 / 178; access checker 4 / 56; ProviderCapabilities 16 / 548 |
...
| 5a | typed conditional-authority gate + Native clone transaction + anchored staged capsule | fail-closed bounded publication | provider authority and internal-APFS metadata/durability complete: Native transaction 15 / 312, ProviderCapabilities 16 / 548, Native create-copy 19 / 924, Native mapper 2 / 382 |
...
| 5c | typed Pool finalizer + production-configured `CopyOperationOrchestrator` | engine-only single-item composition | completed: Pool 15 / 190, orchestrator 13 / 558 with retry/reconcile/release custody; no app consumer |
...
| 5e | app review/configuration + one Copy consumer | `CopyAs` mutation boundary | typed review, cold observer configuration, exact terminal presentation and mutation-boundary proof; cross-volume and real-volume gates remain separate |
...
Current M3 `OperationsUT` evidence passes 165 / 4,468 in Debug and explicit Release ASAN/UBSAN with confirmed runtimes and no diagnostics. Full Debug `VFSUT` passes 94 / 43,531; current-tree M0 remains 897 / 132,011 and Docker-backed ASAN integration remains 163 / 89,392. Hosted CI publication and required-check enforcement remain separate work.
