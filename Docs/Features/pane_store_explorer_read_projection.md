...
- Docker FTP/SFTP/WebDAV navigation/refresh and WebDAV outage/reconnect: 4 cases / 178 assertions; shadow-host mutations drive the fresh listing, FTP and WebDAV cover the exact `Superseded` → soft successor → `Committed` latest-wins chain, SFTP covers direct user-refresh commit, and the WebDAV endpoint restart covers typed `NetworkError`, retained committed state and reconnect;
...
- remaining production coverage: FTP/SFTP transport fault/timeout and FDA/security-scoped permission;
