...
> Status: M1 committed-state read slice, production navigation/refresh lifecycle, `PaneStore` reduction, Explorer breadcrumb projection, and Store-backed View/Back/Forward/Up/Refresh command presentation implemented; remaining pane consumers remain in progress
...
- focused tests and the aggregate WinCommander suite pass.
...
- remaining production coverage: live local/remote provider and permission/deferred-Busy integration;
