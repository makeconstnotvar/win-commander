...
The implemented engine port is not yet a Registry adapter or presentation API:
...
- Registry availability and presentation derive from the same `OperationRecord` revision passed to the engine port; direct command execution revalidates the target and cannot route to a raw pointer.
...
- multi-window aggregation, bounded retention/redaction, compact-view migration and Explorer Operation Center rendering receive focused tests as each consumer lands.
...
This contract selects the model/control authority and cancellation semantics. The implemented state enum is the narrow Copy-foundation subset (`Queued`, `Running`, `Paused`, `Cancelling`, `Finalizing` and terminal outcomes); canonical planning/preparation/wait/retry/rollback states remain future work. It leaves bounded recovery-projection retry/defer UX, Explorer screen, retention/redaction policy, pause/resume engine support, user retry UX, remote transfer adoption and broad mutation-consumer migration to later R6 slices.
