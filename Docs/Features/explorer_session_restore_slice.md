...
The Commander payload remains intact inside the envelope and is restored first as the permanent window base. A valid legacy `panels_v1` root is accepted as Commander mode; the next successful save emits schema v1. Startup always presents the Explorer interface: a decoded Commander-mode session restores only the base and then raises a fresh Explorer state above it, so the persisted mode selects whether Explorer tabs are recovered, not which interface the window opens in. The Commander interface remains reachable through the runtime toggle only. System restoration selects the first valid source in strict order: Cocoa state, `StateConfig`, then the normal default Commander/Home content. Manual restoration uses `StateConfig` when last-window restoration is enabled and no live window supplies the established option-copy path.
