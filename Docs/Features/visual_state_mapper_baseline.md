...
Production `PaneStore` publishes committed empty/loaded state plus reduced navigation/refresh overlays. `PanelController` shares one factory-injected `PaneId` with its Store bridge and controller-owned `PanelControllerLifecycle`; navigation and refresh map correlated worker ownership, cancellation/failure, and post-model-commit success into ordered lifecycle events. Async navigation performs detached fetch on a shared queue and posts only weak-controller main callbacks; intent invalidation cancels its token and dealloc stops without waiting. Persistency recovery callbacks are weak. Refresh physically coalesces one running worker plus latest pending, preserves location generation on commit and publishes typed invalid-location failure before ordinary navigation recovery. The Explorer breadcrumb maps this snapshot: committed path/content survives loading, refreshing and failure; typed error/notice text is localized and exposed through AX. Docker `WinCommanderIT` proves this boundary across FTP/SFTP/WebDAV at 6 / 244: distinct controller and shadow hosts verify initial navigation, a fresh listing after forced user refresh, provider soft-successor completion for FTP/WebDAV, direct forced-refresh completion for SFTP, and endpoint outage/reconnect for all three providers that exposes `NetworkError` while retaining committed content and generation before the fresh successor. FTP and SFTP retain their provider-domain `Unavailable` classification; WebDAV retains `POSIX/ECONNREFUSED` or `ECONNRESET`. `PanelController` also maps a typed provider timeout to `TimeoutError` without erasing the original error; SFTP has a 30-second TCP/session deadline proved by a 150 ms in-process blackhole VFSIT. The later recorded remote-transport slice closes deterministic app-boundary timeout coverage. Footer, item rows, remaining P0 commands and operation surfaces become render adapters as their inputs are connected.

Recorded remote-transport evidence superseded the earlier 6 / 244 snapshot: at that slice Debug `VFSUT` was 95 / 43,563 and `WinCommanderUT` was 321 / 5,206; `IntegrationTests` built; focused Docker `WinCommanderIT` passed 9 / 376. FTP listing, WebDAV blocking-control and SFTP TCP/session paths use 30-second production deadlines with isolated 500 ms seams. Pausing each live endpoint preserves raw FTP `operation_timeout`, SFTP `timeout` and WebDAV `POSIX/ETIMEDOUT`, maps the outcome to `TimeoutError`, retains the committed listing/generation and proves same-controller reconnect after unpause. The SFTP blackhole VFSIT remains 1 / 2; a negative `readdir` fails closed and unavailable/timeout connections are not returned to the SFTP pool. Current full Debug evidence is `VFSUT` 96 / 43,606 and `WinCommanderUT` 333 / 5,376.
